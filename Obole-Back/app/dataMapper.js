...
const dataMapper = {
    ...
    removeDeceased:async(deceasedId)=>{
        ...
        const getDeceased=await db.query("SELECT * FROM deceased WHERE id=$1",[deceasedId]);
        
        if(getDeceased.rows[0].exit_date!=null){
            return false;
        } 

        const updateDeceased=await db.query(`UPDATE deceased SET exit_date=$1 WHERE id=$2 RETURNING firstname,lastname,birth_date,entry_date,exit_date,room_id,burial_permit_date,ritual`,[momentDate,deceasedId]);

      
        const getDeceasedRoom=getDeceased.rows[0].room_id;
        return [getDeceasedRoom,getDeceased.rows[0]];
